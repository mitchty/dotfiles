...
Control variables for output, ALL default to no unless changed to yes.

For things that need to be switched, changing to yes will work. Each subheading with options should have a property attribute to control where things get saved to.

os related, aka which os type are we generating things for
#+results: output-os
| bsd | linux | osx |
| no  | no    | no  |

application options, aka things we may or may not have at the destination
#+results: output-options
| nix | x  | mosh | git | tmux |
| no  | no | no   | no  | no   |

and finally what optional programming languages do we want output files for
#+results: output-languages
| haskell | perl | ruby |
| no      | no   | no   |
...
* ~/.profile
:PROPERTIES:
:header-args: :tangle yes :comments no :padline no :cache yes
:END:
#+BEGIN_SRC sh :tangle profile-test :results replace
# -*- mode: Shell-script; -*-
# Common .profile
...
#+BEGIN_SRC perl :tangle tmp/bin/whoson :results replace :tangle-mode (identity #o755)
#!/usr/bin/env perl
#
...
#+BEGIN_SRC perl :tangle tmp/bin/gecos :results replace :tangle-mode (identity #o755)
#!/usr/bin/env perl
#
...
#+BEGIN_SRC perl :tangle tmp/bin/hatimerun :results replace :tangle-mode (identity #o755)
#!/usr/bin/env perl
#
...
#+BEGIN_SRC perl :tangle tmp/bin/ptree.pl :results replace :tangle-mode (identity #o755)
#!/usr/bin/env perl
#
...
#+BEGIN_SRC perl :tangle tmp/bin/portchk :results replace :tangle-mode (identity #o755)
#!/usr/bin/env perl
#
# Dumb wrapper to get the gecos information from a uid/username.
...
#+BEGIN_SRC sh :tangle tmp/bin/notify :results replace :tangle-mode (identity #o755)
#!/bin/sh
...
#+BEGIN_SRC ruby :tangle tmp/bin/today :results replace :tangle-mode (identity #o755)
#!/usr/bin/env ruby
...
#+BEGIN_SRC conf :tangle tmp/.Xdefaults :results replace
...
#+BEGIN_SRC conf :tangle tmp/.Xmodmap :results replace
pointer = 3 2 1 4 5
#+END_SRC
...
#+BEGIN_SRC conf :tangle tmp/.Xresources :results replace
